...
/// Cretae Popup Child
///Create Popup panel
var _panelID = create_popup_child_panel(anchor.middle, anchor.center, xScale, yScale, obj_pop_up_panel_invite);
...
if(inviteCount > 0)
{
	...
	var _playerUID = ds_map_find_first(inviteMap);

	// Add Items in the List
	for(var a = 0 ; a < inviteCount ; a++)
	{		
		var _ins = create_popup_child_button(_panelID,0,0,8,1,obj_button_invite_details);
		  _ins.image_blend	= color;
		  _ins.playerUID	= _playerUID;
		  _ins.load_player_data();
	
		_playerUID = ds_map_find_next(inviteMap,_playerUID);
