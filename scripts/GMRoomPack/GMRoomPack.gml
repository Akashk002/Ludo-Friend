...
function room_pack_store_layers(l_list){
	/// room_pack_raw_store_layers(list:ds_list<layer>)
	/// @param {ds_list<layer>} list
	/// @returns {void}
	room_pack_raw_store_layers=l_list;
}
...
function room_pack_raw_add_layer(l_ql){
	...
	////// Shubham Edited
	var l_ql_depth =  PopUpDepth  - (global.__popUpLayerNo *50);//l_ql[?"depth"] 
	
	global.__popUpLayerNo++;

	/// Shubham Edited
	var l_ql_name=l_ql[?"name"] + string(ds_stack_size(global.__popUpIDList));	/// Shubham Edited	
	var l_list=room_pack_raw_store_layers;										/// Shubham Edited
	if(l_list!=undefined)ds_list_add(l_list,l_ql_name);							/// Shubham Edited
	room_pack_raw_top_layer = l_ql_name;
...
room_pack_raw_store_backgrounds=undefined;
globalvar room_pack_raw_store_layers; /// @is {ds_list<layers>}
room_pack_raw_store_layers=undefined;
...
globalvar room_pack_raw_top_layer; /// @is {ds_map<string; live_yy_YyRoomInstance>}
room_pack_raw_top_layer=0;
